...
class SheetsDB:
    ...
    def _seed_defaults_sync(self):
        ...
        col_values = [r[4].strip() if len(r) > 4 else "" for r in d_ws.get_all_values()]
        if not any(col_values[1:]):
            rows = [[reason] for reason in DEFAULT_REASONS]
            d_ws.update(values=rows, range_name=f"E2:E{1 + len(rows)}")
        # Настройка времени дайджеста по умолчанию
        s_ws = self._ws[SH_SETTINGS]
        values = s_ws.get_all_values()
        keys = {r[0].strip() for r in values}
        if "digest_time" not in keys:
            s_ws.append_row(["digest_time", self._cfg.default_digest_time], value_input_option="RAW")
    ...
    def _append_employee_sync(self, emp: Employee):
        with self._lock:
            self._ws_of(SH_EMPLOYEES).append_row(emp.to_row(), value_input_option="RAW")
    ...
    def _user_upsert_sync(self, uid, name, role, notifications, added_by):
        with self._lock:
            ...
            if row_num:
                ...
                ws.update_cell(row_num, 3, role)
                ws.update_cell(row_num, 4, "1" if notifications else "0")
            else:
                ws.append_row([uid_s, name, role, "1" if notifications else "0", now, added_by],
    ...
    def _append_row_sync(self, title: str, row: list[str]):
        with self._lock:
            self._ws_of(title).append_row(row, value_input_option="RAW")

    def _get_rows_sync(self, title: str) -> list[list[str]]:
        with self._lock:
            return self._ws_of(title).get_all_values()
